...
eyes_cascade = cv2.CascadeClassifier(os.sep.join([os.environ["OPENCVDATA"],'haarcascades','haarcascade_eye_tree_eyeglasses.xml']))
...
while True:
    ...
    for (X, Y, W, H) in faces:
        ...
        eyes = eyes_cascade.detectMultiScale(faceROI, scaleFactor = 1.1, minNeighbors = 5, minSize = (10, 10), flags = cv2.CASCADE_SCALE_IMAGE)
        # 需注意这里的坐标是在ROI上的坐标
        for (x, y, w, h) in eyes:
            cv2.rectangle(frameCopy, (x+X, y+Y), (x+X + w, y+Y + h),(0, 0, 255), 1)
